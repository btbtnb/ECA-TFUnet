...
def train(Model = None):
    ...
    val_best_iou = 0.3
    #criteon = DiceLoss(num_classes)
    losse = nn.CrossEntropyLoss()
    #criteon = nn.CrossEntropyLoss(weight=torch.from_numpy(np.array([0.03,0.03,0.03,0.5,0.1,3,4,0.03,0.5,1,0.5,0.5])).float())
    lossd = DiceLoss()
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        for val_img, val_mask in tqdm(val_data_loader_num,ncols=20,total=len(val_data_loader_num)):
            ...
            dice += lossd.forward(predict_use,val_use,softmax=True)
            ce += losse.forward(predict_use,val_use.long())
            test_epoch_loss = 0.1*dice + 0.9*ce
